...
namespace Drupal\drupal_ckeditor\Plugin\CKEditorPlugin;
...
class CkeditorTemplates extends CKEditorPluginBase implements CKEditorPluginConfigurableInterface, ContainerFactoryPluginInterface
{
  ...
  {
    return ['drupal_ckeditor/ckeditor.templates.dialog'];
  }
  ...
  public function settingsForm (array $form, FormStateInterface $form_state, Editor $editor)
  {
    ...
    $form['template_path'] = [
      ...
      '#description'   => t('Path to the javascript file defining the templates, relative to drupal root (starting with "/"). By default, it looks in your default theme directory for a file named "templates/ckeditor.js"'),
    ];
    ...
    $form['#attached']['library'][] = 'drupal_ckeditor/ckeditor.templates.admin';
  ...
  private function getTemplatesDefaultPath ()
  {
    ...
    $defaultPath = '/' . drupal_get_path('module', 'drupal_ckeditor') . '/templates/example.js';
    ...
    $defaultThemeFileAbsolutePath = DRUPAL_ROOT . '/' . drupal_get_path('theme', $defaultThemeName) . '/sources/js/' . $defaultThemeName . '_ckeditor.js';

    if (file_exists($defaultThemeFileAbsolutePath)) {
      $defaultPath = '/' . drupal_get_path('theme', $defaultThemeName) . '/sources/js/' . $defaultThemeName . '_ckeditor.js';;
    }
